...
TARGET="/dev/CMSSW_3_2_4/online/\$TABLE"    # no explicit version, take te most recent 
...
function getContentForCVS() {
  ...
  edmConfigFromDB --configName $CONFIG --nopaths --noes --nopsets --noservices --cff --blocks $(getOutputCommands "hltOutputA") --format python > hltOutputA_cff.py
  edmConfigFromDB --configName $CONFIG --nopaths --noes --nopsets --noservices --cff --blocks $(getOutputCommands $OUTPUT_MON)  --format python > hltOutputMON_cff.py
  edmConfigFromDB --configName $CONFIG --nopaths --noes --nopsets --noservices --cff --blocks $(getOutputCommands $OUTPUT_ALCA) --format python > hltOutputALCA_cff.py
}
